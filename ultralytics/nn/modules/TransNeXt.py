...
def get_relative_position_cpb(query_size, key_size, pretrain_size=None):
    # --- 修正 START ---
    # 移除硬性的設備指定，讓張量默認在 CPU 上創建。
    # 當模型整體被 .to(device) 時，這些緩衝區會被一同轉移。
...
if __name__ == '__main__':
    ...
    img_h, img_w = 640, 640
    print("--- Creating TransNeXt Tiny model ---")
    
    # 2. 初始化模型 (此時模型和其內部張量都在 CPU 上)
    model = transnext_micro(img_size=img_h) 
    ...
    print(f"\n--- Testing TransNeXt Tiny forward pass (Input: {input_tensor.shape} on {input_tensor.device}) ---")
    # --- 修正 END ---
    ...
    try:
        ...
        for i, features in enumerate(output_features):
            print(f"Stage {i+1}: {features.shape} on {features.device}") # 應為 [B, C, H_i, W_i]
