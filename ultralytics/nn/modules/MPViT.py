...
class MPViT(nn.Module):
    ...
    def reset_classifier(self, num_classes):
        self.num_classes = num_classes
        self.cls_head = nn.Linear(self.embed_dims[-1], num_classes) if num_classes > 0 else nn.Identity()

    def forward_features(self, x):
        """
        forward feature function
        修改: 返回一個包含各階段特徵圖的列表，以修復 AttributeError
        """
        # x's shape : [B, C, H, W]
        feature_outputs = [] # 用於儲存每個階段的輸出
    ...
    def forward(self, x):
        """
        foward function
        修改: 直接返回特徵列表，以供YOLO等框架使用
        如果需要進行分類，則取列表最後一個元素進行
        """
        x = self.forward_features(x)

        # For classification (original behavior)
        # out = self.cls_head(x[-1]) 
        # return out

        # For detection/segmentation frameworks like ultralytics
        return x


@register_model
def mpvit_tiny(pretrained=False, img_size=224, **kwargs):
    """mpvit_tiny :
    ...
    """
    # pretrained 參數已加入但在此模型中未使用，僅為保持 API 一致性
    model = MPViT(
        img_size=img_size,
        num_stages=4,
        num_path=[2, 3, 3, 3],
...
@register_model
def mpvit_xsmall(pretrained=False, img_size=224, **kwargs):
    """mpvit_xsmall :
...
@register_model
def mpvit_small(pretrained=False, img_size=224, **kwargs):
    """mpvit_small :
...
@register_model
def mpvit_base(pretrained=False, img_size=224, **kwargs):
    """mpvit_base :
...
if __name__ == "__main__":
    model = mpvit_xsmall(img_size=224)
    model.eval()
    ...
    # 預期輸出是一個包含4個特徵圖的列表
    features_list = model(inputs)

    print(f"Output is a list: {isinstance(features_list, list)}")
    print(f"Number of feature maps: {len(features_list)}")
    for i, features in enumerate(features_list):
        print(f"  - Feature map {i+1} shape: {features.shape}")

    # 測試 width_list 是否成功建立
    print(f"Width list: {model.width_list}")
    
    # 測試分類頭 (如果需要的話)
    # classification_output = model.cls_head(features_list[-1])
    # print(f"Classification head output shape: {classification_output.shape}")
    ...
    try:
        ...
        # FlopCountAnalysis 需要一個單一的輸出張量，
        # 所以我們需要包裝一下模型來進行分析
        class ModelForAnalysis(nn.Module):
            def __init__(self, model):
                super().__init__()
                self.model = model
            def forward(self, x):
                return self.model(x)[-1] # 只分析到最後一層特徵

        analysis_model = ModelForAnalysis(model)
        flops = FlopCountAnalysis(analysis_model, inputs)
        param = sum(p.numel() for p in model.parameters() if p.requires_grad)
        acts = ActivationCountAnalysis(analysis_model, inputs)

        print(f"total flops : {flops.total()}")
        print(f"total activations: {acts.total()}")
        print(f"number of parameter: {param}")
    except ImportError:
        print("fvcore not installed, skipping complexity analysis.")